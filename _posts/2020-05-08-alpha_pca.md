...
excerpt: "Exploring volatility through a linear algebraic approach, commonly known as Principal Component Analysis (PCA) in various other non-finance purposes."
mathjax: "true"
---
...
<img src="https://i.ibb.co/VCXtjNb/3.png" alt="3" border="0">
...
Combining all the written codes above, we compile a **single function** for 2 & 3 dimensional data (to be modified for more functionalities below):
...
def EIGEN(ret):
    ...
    _labels = pd.Series([(i+" - "+str(
                    (explained_variance_ratio[i]*100).round(2))+"%") for i in explained_variance_ratio.index],
...
Calling it on our demonstrated example of SPY v.s. IEF, we obtain:
